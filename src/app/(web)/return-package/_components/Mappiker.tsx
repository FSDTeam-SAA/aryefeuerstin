...
// Default & fallback location (UK - London)
const UK_LOCATION = {
  lat: 51.5074,
  lng: -0.1278,
};
...
export default function LocationPickerModal({
  ...
  // Map center position (default: UK)
  const [position, setPosition] = useState<{ lat: number; lng: number }>(
    UK_LOCATION
  );
  ...
  /* =====================================================
     ...
     - Else try to get user's current location
     - If permission denied → keep UK location
     ===================================================== */
  useEffect(() => {
    if (initialLocation) {
      setPosition({ lat: initialLocation.lat, lng: initialLocation.lng });
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        // Permission denied → keep UK location
        setPosition(UK_LOCATION);
      },
  ...
  const handlePlacesChanged = () => {
    ...
    const locationData = {
      address: places[0].name || "Selected Location",
      lat,
      lng,
    };
        ...
        >
          ...
          <div className="absolute top-3 left-3 z-10 flex flex-col sm:flex-row gap-2 w-[calc(100%-1rem)] sm:w-auto">
            ...
            <button
              ...
              onClick={handleGetMyLocation}
              disabled={isGettingLocation}
              className="w-full sm:w-auto h-10 px-4 rounded bg-white border border-gray-300 shadow-md flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {isGettingLocation ? (
                <Loader2 className="animate-spin text-blue-500" size={18} />
              ) : (
