  ...
  const handleMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        // Pan and zoom the map to current location
        if (mapRef.current) {
  ...
  return (
    <div className="space-y-3">
      <div className="h-[500px] w-full rounded-md border overflow-hidden relative">
        ...
        >
          <div className="absolute top-3 left-3 z-10 flex flex-col gap-2 w-[calc(100%-1rem)] sm:w-auto">
            <StandaloneSearchBox
              onLoad={(ref) => setSearchBox(ref)}
              onPlacesChanged={handlePlacesChanged}
            >
            ...
            <button
              ...
              disabled={isLocating}
              className="w-full sm:w-auto px-4 h-10 rounded border border-gray-300 shadow-md bg-white text-sm font-medium hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {isLocating ? (
                <>
