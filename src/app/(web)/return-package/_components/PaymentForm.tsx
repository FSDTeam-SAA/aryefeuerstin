...
export function SummaryReview({
  ...
  // Auto-scroll on mobile – adjusted to scroll a bit higher
  useEffect(() => {
    if (window.innerWidth >= 768) return;

    const timer = setTimeout(() => {
      const totalSection = document.querySelector(
        '.border-2\\.border-\\[\\#31B8FA\\]'
      );
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Conditional Rendering Based on Total Amount – মোবাইলে ছোট করা */}
      {isFreeOrder ? (
        <div className="text-center py-12">
          <div className="inline-flex items-center justify-center w-20 h-20 bg-green-100 rounded-full mb-6">
            <CheckCircle className="w-12 h-12 text-green-600" />
        ...
        <>
          ...
          <div className="border-2 border-[#31B8FA] rounded-xl p-4 sm:p-6 bg-gradient-to-r from-cyan-50 to-blue-50 shadow-lg">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-0">
              ...
              <p className="text-2xl sm:text-3xl font-extrabold text-[#31B8FA]">
                ${totalAmount.toFixed(2)}
              </p>
            </div>
          </div>

          {/* Payment Button – মোবাইলে ছোট + full width */}
      ...
      {/* Payment Error Message */}
      {!isFreeOrder && checkoutMutation.isError && (
        <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-lg text-center mt-6">
          <p className="font-medium">
            {(checkoutMutation.error as Error)?.message ||
              "Payment initiation failed. Please try again."}
          </p>
