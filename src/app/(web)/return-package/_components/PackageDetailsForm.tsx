...
export function PackageDetailsForm({
  ...
  const updateStoreData = useCallback((index: number, field: keyof StoreData, value: any) => {
    setStores((prev) => {
      const updated = [...prev]
      // @ts-ignore
      updated[index][field] = value
  ...
  const handleImageChange = useCallback(async (
    ...
    try {
      ...
      setStores((prev) => {
        const updated = [...prev]
        updated[storeIdx].packageImages[pkgIdx] = [compressedDataUrl]
        return updated
      })
    } catch (error) {
  ...
  const removeImage = useCallback((storeIdx: number, pkgIdx: number) => {
    setStores((prev) => {
      const updated = [...prev]
      updated[storeIdx].packageImages[pkgIdx] = []
      return updated
  ...
  const handlePhysicalLabelUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files
    if (!files) return
  ...
  const removePhysicalLabelFile = useCallback((index: number) => {
    setPhysicalReturnLabelFiles((prev) => {
      if (prev[index]?.preview) URL.revokeObjectURL(prev[index].preview!)
      return prev.filter((_, i) => i !== index)
    })
  ...
  const onSubmit = useCallback(async (data: PackageDetailsFormData) => {
    await onSubmitAndProceed({ ...data, stores, physicalReturnLabelFiles })
  }, [stores, physicalReturnLabelFiles, onSubmitAndProceed])
