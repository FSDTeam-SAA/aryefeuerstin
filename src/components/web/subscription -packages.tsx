            ...
            {isLoading ? (
              ...
              plans.map((pkg, index) => {
                ...
                return (
                  <div
                    key={pkg._id}
                    className={`border-2 ${colors.border} rounded-lg overflow-hidden flex flex-col`}
                  >
                    {/* Header */}
                    <div
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                      <h4 className="text-center text-[28px] font-semibold mb-6">
                        {pkg.name}
                      </h4>
                      ...
                      <ul className="space-y-2 mb-6">
                        {pkg.features.map((feature, i) => {
                          const isImportant =
                            /priority|support|unlimited|recommended/i.test(
                              feature
                            );
                          return (
                            <li
                      ...
                      <Button
                        disabled={mutation.isPending}
                        onClick={() => handleJoinNow(pkg._id)}
                        className={`${colors.button} text-white h-[51px] rounded-[8px] w-full mt-auto`}
                      >
                        {mutation.isPending &&
                        (mutation.variables as { planId: string })?.planId ===
                          pkg._id ? (
