...
const createCheckoutSession = async ({ planId, token }: { planId: string; token?: string }): Promise<CheckoutResponse> => {
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/subscription/buy/${planId}`, {
    method: "POST",
    headers: { 
...
export default function SubscriptionPackages() {
  ...
  const handleJoinNow = (planId: string) => {
    ...
    // Pass both planId and token to the mutation
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {isLoading ? (
            ...
            plans.map((pkg) => (
              <div key={pkg._id} className="border-2 border-[#3258DA] rounded-lg overflow-hidden flex flex-col">
                <div className="bg-black text-white p-6 text-center">
                  <h3 className="font-semibold text-2xl uppercase">{pkg.title}</h3>
                </div>

                <div className="p-6 flex-1 flex flex-col justify-center">
                  <div className="flex justify-center mb-6 text-center">
                    <div>
                      <h4 className="text-[32px] font-semibold text-[#131313] mb-1">${pkg.price}</h4>
                      <p className="text-base text-[#424242]">{pkg.name}</p>
                    </div>
                  </div>
                  ...
                  <div className="mb-6">
                    ...
                    <ul className="space-y-2 mt-5">
                      {pkg.features.map((feature, index) => (
                        <li key={index} className="text-sm font-normal text-[#000000] flex items-start gap-2">
                  ...
                  >
                    {mutation.isPending && (mutation.variables as { planId: string })?.planId === pkg._id ? (
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    ) : null}
                    {pkg.price === 0 ? "Request Now" : `Join Now - $${pkg.price}/${pkg.billingCycle === 'monthly' ? 'mo' : 'yr'}`}
                  </Button>
                </div>
              </div>
            ))
